...
const { findByIdAndDelete } = require("../models/categorie");
        ...
        try {
            ...
            if(cartItem){
                ...
                if(cartItem.quantity > 1){
                    ...
                    await cartItem.save();
                    res.status(200).json({status: true, message: 'Quantité de produit diminue avec success'});
                }else{
                    await Cart.findByIdAndDelete(id);
                    res.status(200).json({status: true, message: 'Produit supprimé avec success'});
                };

            }else{
                res.status(404).json({status: false, message: 'Produit introuvable'}); 
            }
        } catch (error) {
            res.status(500).json({status: false, message: error.message});
        }
